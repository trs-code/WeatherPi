...
int main()
{
    ...
    layer* denseLayer0 = make_dense_layer((layer**[]){&inLayer0}, 3, 1, 1);
    if(denseLayer0 == NULL) goto error4;

    layer* denseLayer1 = make_dense_layer((layer**[]){&inLayer1}, 3, 1, 1);
    if(denseLayer1 == NULL) goto error5;

    layer* denseLayer2 = make_dense_layer((layer**[]){&inLayer2}, 3, 1, 1);
    if(denseLayer2 == NULL) goto error6;

    layer* denseLayer3 = make_dense_layer((layer**[]){&denseLayer0, &denseLayer1, &denseLayer2}, 9, 3, 1);
    if(denseLayer3 == NULL) goto error7;

    layer* denseLayer4 = make_dense_layer((layer**[]){&denseLayer3}, 3, 1, 1);
    if(denseLayer4 == NULL) goto error8;

    layer* outLayer = make_output_layer((layer**[]){&denseLayer4}, 1, 1);
    if(denseLayer2 == NULL) goto error9;

    layer* inLayers[] = {inLayer0, inLayer1, inLayer2};
    model *wethrModel = construct_model((layer**[]){&inLayer0, &inLayer1, &inLayer2}, &outLayer, 9, 3, 0.1f);
    if(wethrModel == NULL) goto error10;

    read_csv(filename, numSamples, numIns, numOuts, &inArrays, &outArrays);

    train_model_sgd(wethrModel, 500, numSamples, inArrays, outArrays);
