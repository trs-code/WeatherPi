...
int main()
{
    ...
    layer* denseLayer0 = make_hidden_layer((layer**[]){&inLayer0}, 9, 1, 't');
    if(denseLayer0 == NULL) goto error4;

    layer* denseLayer1 = make_hidden_layer((layer**[]){&inLayer1}, 9, 1, 't');
    if(denseLayer1 == NULL) goto error5;

    layer* denseLayer2 = make_hidden_layer((layer**[]){&inLayer2}, 9, 1, 't');
    if(denseLayer2 == NULL) goto error6;

    layer* denseLayer3 = make_hidden_layer((layer**[]){&denseLayer0, &denseLayer1, &denseLayer2}, 36, 3, 't');
    if(denseLayer3 == NULL) goto error7;

    layer* denseLayer4 = make_hidden_layer((layer**[]){&denseLayer3}, 18, 1, 't');
    if(denseLayer4 == NULL) goto error8;

    layer* outLayer = make_output_layer((layer**[]){&denseLayer4}, 1, 1, 's');
    if(denseLayer2 == NULL) goto error9;

    model *wethrModel = construct_model((layer**[]){&inLayer0, &inLayer1, &inLayer2}, &outLayer, 9, 3, 0.000001f, 'b');
