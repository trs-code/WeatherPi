...
int main()
{
    ...
    layer* denseLayer0 = make_hidden_layer((layer**[]){&inLayer0}, 16, 1, 't');
    if(denseLayer0 == NULL) goto error4;

    layer* denseLayer1 = make_hidden_layer((layer**[]){&inLayer1}, 16, 1, 't');
    if(denseLayer1 == NULL) goto error5;

    layer* denseLayer2 = make_hidden_layer((layer**[]){&inLayer2},  16, 1, 't');
    if(denseLayer2 == NULL) goto error6;

    layer* denseLayer3 = make_hidden_layer((layer**[]){&denseLayer0, &denseLayer1, &denseLayer2}, 64, 3, 't');
    if(denseLayer3 == NULL) goto error7;

    layer* denseLayer4 = make_hidden_layer((layer**[]){&denseLayer3}, 32, 1, 't');
    ...
    train_model_sgd_fast(wethrModel, 10, numSamples, inArrays, outArrays, 0.8);
